...
import { AudioComponent } from '@/lib/exercise/exercise.types';

interface Props { item: AudioComponent; onChange: (updated: AudioComponent) => void; }

export function AudioEditor({ item, onChange }: Props) {
  ...
  return (
    <div className="flex flex-col gap-admin-md">
      ...
      <label className="flex flex-col gap-1">
        <span className="text-admin-xs text-admin-text-muted">URL zvuku</span>
        <input
      ...
      <label className="flex flex-col gap-1">
        ...
        <input
          ...
          placeholder="Název audio..."
        />
      </label>
      {item.url && <audio src={item.url} controls className="w-full" />}
    </div>
  );
}
