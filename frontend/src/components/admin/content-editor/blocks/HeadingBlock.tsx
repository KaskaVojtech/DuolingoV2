...
import { HeadingBlock as HBlock } from '@/lib/content-editor/content-editor.types';
...
export function HeadingBlock({ block, isSelected, onSelectionChange }: Props) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); addBlock('paragraph', block.id); }
    if (e.key === 'Backspace' && ref.current?.innerHTML === '') { e.preventDefault(); deleteBlock(block.id); }
  };

  const fontSize = (block as HBlock & { fontSize?: number }).fontSize ?? getHeadingFontSize(block.level);
